'''
From the paper "Image Segmentation Metric and its Application in the Analysis of Material Microscopic Image"
@article{ma2021image,
  title={Image segmentation metric and its application in the analysis of microscopic image},
  author={Ma, BY and Jiang, SF and Yin, D and others},
  journal={Chin J Eng},
  volume={43},
  number={1},
  pages={137},
...
def get_pixel_accuracy(pred: np.ndarray, mask: np.ndarray) -> float:
    ...
    temp_n_ii = 0.0
    temp_t_i = 0.0
    for i_cl in range(int(class_num)):
...
def get_fwiou(pred: np.ndarray, mask: np.ndarray) -> float:
    ...
    temp_t_i = 0.0
    temp_iou = 0.0
    for i_cl in range(0, int(class_num)):
        n_ii = np.count_nonzero(mask[pred == i_cl] == i_cl)
        t_i = np.count_nonzero(mask == i_cl)
